function validateForm(form){
	...
	if(numAtiv == INICIO_SOLICITACAO || numAtiv == ATIVIDADE_INICIO){
		...
		if(form.getValue("dsAnaliseCausa") == null || form.getValue("dsAnaliseCausa").trim().isEmpty()){
		    ...
		    if(indexes.length == 0){
		        ...
		        for (var i = 0; i < indexes.length; i++) {
		            ...
		            (form.getValue("w3___" + indexes[i]) == null || form.getValue("w3___" + indexes[i]).trim().isEmpty()) &&
		            (form.getValue("w4___" + indexes[i]) == null || form.getValue("w4___" + indexes[i]).trim().isEmpty()) && 
		            (form.getValue("w5___" + indexes[i]) == null || form.getValue("w5___" + indexes[i]).trim().isEmpty()) &&
		            (form.getValue("h1___" + indexes[i]) == null || form.getValue("h1___" + indexes[i]).trim().isEmpty())&&
		            (form.getValue("h2___" + indexes[i]) == null || form.getValue("h2___" + indexes[i]).trim().isEmpty())){
		            	errorMsg += "-Ao menos um dos campos do 5W2H deve ser preenchido!"+lineBreaker;
		            }
		...
		var indexes = form.getChildrenIndexes("ativPlano");
        for (var i = 0; i < indexes.length; i++) {
        	if(form.getValue("cdResponsavel___" + indexes[i]) == getValue("WKUser") && (form.getValue("situacao___" + indexes[i]) == null || form.getValue("situacao___" + indexes[i]).trim().isEmpty())){
...
//	            }
//	            if(form.getValue("w2___" + indexes[i]) == null || form.getValue("w2___" + indexes[i]).trim().isEmpty()){
//	            	errorMsg += "O campo QUEM/Responsável é obrigatório!"+lineBreaker;
//	            }
...
//	            }
//	            if(form.getValue("situacao___" + indexes[i]) == null || form.getValue("situacao___" + indexes[i]).trim().isEmpty()){
//	            	errorMsg += "O campo ACOMPANHAMENTO DO PLANO é obrigatório!"+lineBreaker;
//	            }
//	        }
//	    }
//	}
